window.huuperEventDetail = (() => {
  ...
  function init(config) {
    ...
    if (!summaryNode || !tabsNode || !tabUsersNode || !tabGuestsNode || !tabPendingNode || !registrationsNode || !cancelledBlockNode || !cancelledToggleNode || !cancelledLabelNode || !cancelledCountNode || !cancelledListNode || !window.huuperAuth || !window.huuperListPage || !window.huuperEventSummary || !window.huuperListItem || !window.huuperActionSheet) {
      return;
    }
    ...
    window.huuperAuth.apiFetch(config.detailURL(id)).then((payload) => {
      ...
      summaryNode.hidden = false;
      summaryNode.innerHTML = window.huuperEventSummary.render(location, duration);
        ...
        if (items.length > 0 || pending.length > 0 || cancelled.length > 0) {
          ...
          const renderItems = (collection) => {
            window.huuperListPage.renderList(registrationsNode, collection, (item) => {
              ...
              return window.huuperListItem.renderAttendee(item, href, { menu: allowMenu });
            });
