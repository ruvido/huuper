window.huuperUserDetail = (() => {
  ...
  function renderGroups(node, groups, scope) {
    ...
    window.huuperListPage.renderList(node, groups, (group) => {
      return window.huuperListPage.renderCompactLink(`/${scope}/group/?id=${encodeURIComponent(group.id)}`, group.name || group.id, window.huuperListPage.text(group.type));
    });
  }
  ...
  function init(config) {
    ...
    window.huuperAuth.apiFetch(config.detailURL(id)).then((payload) => {
      ...
      for (const request of guardianRequests) {
        rows.push(window.huuperRequestCard.renderCompact(request, `/${config.scope}/request/?id=${encodeURIComponent(request.id)}`));
      }
