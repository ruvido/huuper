(() => {
  const listNode = document.querySelector("#admin-requests-list");
  const statusNode = document.querySelector("#admin-requests-status");
  ...
  async function load() {
    try {
      const payload = await window.huuperAuth.apiFetch("/api/admin/requests");
