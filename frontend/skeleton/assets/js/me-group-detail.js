(() => {
  ...
  window.huuperAuth.apiFetch(`/api/me/groups/${encodeURIComponent(id)}`).then((group) => {
    window.huuperListPage.setStatus(statusNode, "");
    summaryNode.hidden = false;
    summaryNode.innerHTML = `<article><strong>${window.huuperListPage.escapeHTML(group.name || id)}</strong><p class="status">${[group.type, `${group.members_count} members`, group.requests_visible && group.requests_count > 0 ? `${group.requests_count} requests` : ""].filter(Boolean).join(" • ")}</p></article>`;
    ...
    if (memberItems.length > 0) {
      ...
      window.huuperListPage.renderList(membersNode, memberItems, (item) => {
        ...
        return `<a href="/me/user/?id=${encodeURIComponent(item.id)}"><strong>${window.huuperListPage.escapeHTML(item.full_name || item.email || item.id)}</strong>${meta.length > 0 ? `<p class="status">${meta.join(" • ")}</p>` : ""}</a>`;
      });
    }
  }).catch(() => {
