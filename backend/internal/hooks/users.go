...
func RegisterUsersAuthGate(app *pocketbase.PocketBase) {
	app.OnRecordAuthRequest("users").BindFunc(func(e *core.RecordAuthRequestEvent) error {
		...
		if strings.TrimSpace(backendinternal.AnyToString(data["onboarding_completed_at"])) == "" {
			...
			if err != nil {
				return apis.NewForbiddenError("onboarding_incomplete", map[string]any{"error": backendinternal.RawErrorValue{Value: err.Error()}})
			}
			return apis.NewForbiddenError("onboarding_incomplete", map[string]any{
				"onboarding_url": backendinternal.RawErrorValue{Value: backendrequests.BuildRelativeOnboardingURL(token)},
			})
		}
		return e.Next()
	})
